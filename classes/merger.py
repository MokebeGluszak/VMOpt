...
class Merger:
    ...
    def export_merge_info(self, merged_df: pd.DataFrame, error_caption: str) -> None:
        export_dir = self.get_export_dir().path
        export_df(self.df1, "df1", enum.FileType.XLSX, export_folder_path=export_dir)
        export_df(self.df2, "df2", enum.FileType.XLSX, export_folder_path=export_dir)
